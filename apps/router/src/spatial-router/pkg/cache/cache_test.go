...
func milvusIntegrationEnabled() bool {
	...
	return os.Getenv("RUN_MILVUS_TESTS") == "true"
}

func skipMilvusIntegrationT(t *testing.T) {
	t.Helper()
	if !milvusIntegrationEnabled() {
		t.Skip("Skipping Milvus-dependent test (set RUN_MILVUS_TESTS=true to enable)")
	}
}

func skipMilvusIntegrationGinkgo() {
	if !milvusIntegrationEnabled() {
		Skip("Milvus tests skipped (set RUN_MILVUS_TESTS=true to enable)")
	}
}
