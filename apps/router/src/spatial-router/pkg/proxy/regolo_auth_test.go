					...
					cfg := &config.RouterConfig{
						...
						BackendModels: config.BackendModels{
							ModelConfig: map[string]config.ModelParams{"qwen3.5-9b": {
								UseClientKey: true, PreferredEndpoints: []string{"private-gateway"}, AccessKeyEnv: "REGOLO_API_KEY",
							}},
...
func TestRegoloWithoutServerCredentialUsesClient(t *testing.T) {
	for _, endpoint := range []string{"https://api.regolo.ai/v1", "http://private-gateway/v1"} {
		cfg := &config.RouterConfig{BackendModels: config.BackendModels{
			ModelConfig:       map[string]config.ModelParams{"model": {UseClientKey: true, PreferredEndpoints: []string{"gateway"}}},
...
func TestCodexClassifierProbeUsesConfiguredCredential(t *testing.T) {
	var calls atomic.Int32
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
		if r.Header.Get("Authorization") != "Bearer server-key" {
			t.Error("Codex classifier probe must use the configured credential")
		}
		fmt.Fprint(w, `{"data":[]}`)
	}))
	defer upstream.Close()
	...
	w := httptest.NewRecorder()
	(&Server{cfg: cfg}).handleDiagClassifier(w, httptest.NewRequest(http.MethodGet, "/diag/classifier", nil))
	if calls.Load() != 1 || !strings.Contains(w.Body.String(), `"reachable":true`) {
		t.Fatalf("Codex classifier probe did not use the configured credential: %s", w.Body.String())
	}
}
