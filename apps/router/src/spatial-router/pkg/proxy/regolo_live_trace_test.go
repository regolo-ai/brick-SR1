...
func TestRegoloLiveTrace(t *testing.T) {
	...
	checkpoint, err := os.ReadFile(filepath.Join(config.ResolveModelPath(cfg.SkillRouter.CapabilityModel.ModelID), "config.json"))
	if err != nil {
		...
		t.Run(fmt.Sprintf("stream=%v", streaming), func(t *testing.T) {
			...
			http.DefaultTransport = regoloWireTransport(func(r *http.Request) (*http.Response, error) {
				...
				if classifier {
					...
					inferenceCalls++
					if trace.result == nil || out["model"] != trace.result.Model || !cfg.ModelUsesRegolo(trace.result.Model) {
