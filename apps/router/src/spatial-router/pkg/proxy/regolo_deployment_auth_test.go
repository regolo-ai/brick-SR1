	...
	for _, multimodal := range []bool{false, true} {
		for _, enabled := range []bool{false, true} {
			t.Run(fmt.Sprintf("multimodal=%v/reasoning=%v", multimodal, enabled), func(t *testing.T) {
				cfg, err := config.Parse("../../../../../../deploy/docker-compose/config.regolo.yaml")
				if err != nil {
					t.Fatal(err)
				}
...
func TestRegoloAnthropicAdapterUsesCaller(t *testing.T) {
	...
	cfg.SkillRouter.Models = []config.SkillRouterModelConfig{{Model: "qwen3.5-9b", BaseURL: "https://api.regolo.ai/v1", APIKey: "server-decoy-key"}}
