...
func (s *Server) handleCodexResponses(w http.ResponseWriter, r *http.Request) {
	...
	if d.Protocol == "chat_completions" {
		...
		s.forwardCodexWithHistory(w, r, model, selectedEffort, requestedModel == "brick", func(sw http.ResponseWriter) {
...
func (s *Server) forwardCodexWithHistory(w http.ResponseWriter, r *http.Request, model, effort string, routed bool, forward func(http.ResponseWriter)) {
	...
	if err := s.callHistory.append(callRecord{CallID: newBrickCallID(), StartedAt: started, FinishedAt: time.Now().UTC(), Model: model, ReasoningMode: effort, RoutingMode: mode, RoutingSource: source, Status: status, Error: sanitizeCallError(cause)}); err != nil {
