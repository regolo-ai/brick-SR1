...
// This test requires:
// 1. OpenAI-compatible omni running on localhost:8001
// 2. Spatial Router running on localhost:8080
//
// Run with: go test -tags=integration -v ./pkg/imagegen/...
func TestImageGenerationE2E(t *testing.T) {
	// Check if OpenAI-compatible omni is running
	if !isServiceRunning("http://localhost:8001/health") {
		t.Skip("OpenAI-compatible omni not running on localhost:8001, skipping integration test")
	}
	...
	tests := []struct {
		...
		{
			name:     "Direct OpenAI-compatible omni image generation",
...
func TestMain(m *testing.M) {
	...
	fmt.Println("  - OpenAI-compatible omni on localhost:8001")
