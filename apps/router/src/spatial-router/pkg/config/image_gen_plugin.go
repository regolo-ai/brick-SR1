...
//
// openai_compatible_omni fields: BaseURL, Model, NumInferenceSteps, CFGScale, Seed
// openai fields:    APIKey, Model, Quality, Style
type ImageGenBackendEntry struct {
	// Type identifies the image generation provider: "openai_compatible_omni" or "openai"
	...
	// --- openai_compatible_omni-specific fields ---

	// BaseURL for OpenAI-compatible omni server (e.g., "http://localhost:8001")
...
func (e *ImageGenBackendEntry) ToPluginConfig() *ImageGenPluginConfig {
	...
	case "openai_compatible_omni":
		cfg.BackendConfig = &OpenAICompatibleOmniImageGenConfig{
			BaseURL:           e.BaseURL,
			Model:             e.Model,
...
type ImageGenBackendConfig struct {
	// Backend type: "openai_compatible_omni", "openai"
...
type ImageGenPluginConfig struct {
	...
	// Backend type: "openai_compatible_omni", "openai", "replicate"
...
// OpenAICompatibleOmniImageGenConfig represents configuration for OpenAI-compatible omni image generation
type OpenAICompatibleOmniImageGenConfig struct {
	// Base URL for OpenAI-compatible omni server (e.g., "http://localhost:8001")
...
func (c *ImageGenPluginConfig) UnmarshalBackendConfig() error {
	...
	switch c.Backend {
	case "openai_compatible_omni":
		typedConfig = &OpenAICompatibleOmniImageGenConfig{}
	case "openai":
...
func (d *Decision) GetImageGenConfig() *ImageGenPluginConfig {
	...
	if result.BackendConfig != nil && result.Backend != "" {
		...
		case "openai_compatible_omni":
			backendConfig = &OpenAICompatibleOmniImageGenConfig{}
		case "openai":
...
func (c *ImageGenPluginConfig) Validate() error {
	...
	case "openai_compatible_omni":
		if c.BackendConfig == nil {
			return fmt.Errorf("backend_config is required for openai_compatible_omni backend")
		}
		omniConfig, ok := c.BackendConfig.(*OpenAICompatibleOmniImageGenConfig)
		if !ok {
			return fmt.Errorf("backend_config must be OpenAICompatibleOmniImageGenConfig for openai_compatible_omni backend")
		}
		if omniConfig.BaseURL == "" {
			return fmt.Errorf("base_url is required for openai_compatible_omni backend")
		}
	case "openai":
		if c.BackendConfig == nil {
