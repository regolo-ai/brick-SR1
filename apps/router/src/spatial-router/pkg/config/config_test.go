...
`
			...
			It("should load configuration successfully", func() {
				...
				// Verify OpenAI-compatible endpoints config
				Expect(cfg.ProviderEndpoints).To(HaveLen(2))
				Expect(cfg.ProviderEndpoints[0].Name).To(Equal("endpoint1"))
				Expect(cfg.ProviderEndpoints[0].Address).To(Equal("127.0.0.1"))
				Expect(cfg.ProviderEndpoints[0].Port).To(Equal(8000))
				Expect(cfg.ProviderEndpoints[0].Weight).To(Equal(1))

				Expect(cfg.ProviderEndpoints[1].Name).To(Equal("endpoint2"))
				Expect(cfg.ProviderEndpoints[1].Address).To(Equal("127.0.0.1"))
				Expect(cfg.ProviderEndpoints[1].Weight).To(Equal(2))
...
`
			...
			It("should parse inline milvus backend connection configuration correctly", func() {
				...
				Expect(cfg.SemanticCache.Milvus.Connection.Database).To(Equal("spatial_router_cache"))
