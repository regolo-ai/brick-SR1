...
var _ = Describe("ImageGenPluginConfig.Validate", func() {
	...
	Context("with openai_compatible_omni backend", func() {
		It("passes with valid openai_compatible_omni + valid modality detection", func() {
			cfg := &ImageGenPluginConfig{
				Enabled:       true,
				Backend:       "openai_compatible_omni",
				BackendConfig: &OpenAICompatibleOmniImageGenConfig{BaseURL: "http://localhost:8001"},
