...
const (
	// APIFormatOpenAI is the default OpenAI-compatible API format.
...
type RouterConfig struct {
	...
	*/
	...
	// Brick-specific extensions for HTTP proxy mode
	BrickExtension `yaml:",inline"`

	// SkillRouter configures Brick2's native mathematical router.
...
// routing to local OpenAI-compatible backends that don't require auth).
//
...
type AuthzConfig struct {
	...
	//                    for local OpenAI-compatible backends that don't require auth.
...
type Signals struct {
	...
	// ComplexityService configures the external NVIDIA complexity classification service.
	// When enabled, the router calls the service to get a prompt_complexity_score (0-1)
	// and maps it to easy/medium/hard labels for the decision engine.
...
type BackendModels struct {
	...
	// Provider endpoints configuration for multiple backend support.
	...
	// openai_compatible_omni and openai use different APIs; each entry's Type
...
// This enables running the preference classifier without an external endpoint.
...
type ExternalModelConfig struct {
	// Provider name for the external model server.
	...
	ModelEndpoint LLMEndpoint `yaml:"llm_endpoint,omitempty"`
...
// LLMEndpoint represents an OpenAI-compatible endpoint used by auxiliary classifiers.
// This is separate from ProviderEndpoint, which is used for backend inference.
type LLMEndpoint struct {
	// Address of the endpoint (IP address or hostname)
	Address string `yaml:"address"`
...
// ProviderEndpoint represents a backend provider endpoint.
type ProviderEndpoint struct {
	...
	// +optional
	// +kubebuilder:default=openai_compatible
...
type ModelParams struct {
	...
	// These must be registered with the model server before routing.
	...
	// or "omni" (can handle both text and image generation in a single request, e.g. an omni model
...
type LoRAAdapter struct {
	// Name of the LoRA adapter (must match the name registered with the model server)
	Name string `yaml:"name"`
...
type ReMoMAlgorithmConfig struct {
	...
	// When true, extracts provider reasoning fields and includes them in synthesis prompts
	// Default: false
...
type ComplexityServiceConfig struct {
	...
	// AutoSpawn controls whether brick auto-launches the brick-complexity
...
type SkillRouterMathConfig struct {
	PriorStrength     float64            `yaml:"prior_strength,omitempty"`
