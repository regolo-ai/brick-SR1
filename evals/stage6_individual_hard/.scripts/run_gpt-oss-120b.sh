...
export USAGE_LOG_PATH="/root/forkGO/semantic-routing/evals/stage6_individual_hard/gpt-oss-120b_usage.jsonl"
...
PYTHONPATH="/root/forkGO/semantic-routing/evals" \
  ...
  --output_path '/root/forkGO/semantic-routing/evals/stage6_individual_hard/gpt-oss-120b' \
  --log_samples \
  --batch_size 1 \
  --apply_chat_template \
  --trust_remote_code \
  --system_instruction 'For multiple choice questions, end your response with "the answer is (X)" where X is the letter.' \
  2>&1 | tee '/root/forkGO/semantic-routing/evals/stage6_individual_hard/gpt-oss-120b.log'

echo ""
echo "=== DONE: gpt-oss-120b ==="

# Generate per-model cost summary
if [[ -f "/root/forkGO/semantic-routing/evals/stage6_individual_hard/gpt-oss-120b_usage.jsonl" ]]; then
    python3 "/root/forkGO/semantic-routing/evals/summarize_costs.py" \
        --usage "/root/forkGO/semantic-routing/evals/stage6_individual_hard/gpt-oss-120b_usage.jsonl" \
        --results-dir "/root/forkGO/semantic-routing/evals/stage6_individual_hard/gpt-oss-120b" \
        --output "/root/forkGO/semantic-routing/evals/stage6_individual_hard/gpt-oss-120b_cost_summary.json" \
