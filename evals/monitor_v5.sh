#!/usr/bin/env bash
# monitor_v5.sh — live progress monitor for evals-hard-v5
SESSION="evals-hard-v5"
MODELS=(gpt-oss-120b gpt-oss-20b qwen3-8b Llama-3.3-70B-Instruct mistral-small3.2 qwen3-coder-next)

while true; do
    clear
    echo "╔══════════════════════════════════════════════════════════════════╗"
    echo "║           evals-hard-v5  —  MMLU-Pro 200q  —  $(date '+%H:%M:%S')          ║"
    echo "╠══════════════════════════════════════════════════════════════════╣"
    printf "║  %-28s  %-10s  %-20s  ║\n" "MODEL" "PROGRESS" "ETA"
    echo "╠══════════════════════════════════════════════════════════════════╣"
    ...
    for idx in "${!MODELS[@]}"; do
        model="${MODELS[$idx]}"
        line=$(tmux capture-pane -t "${SESSION}:${idx}" -p 2>/dev/null | grep -E "Requesting API:|=== DONE|\[WARN\]" | tail -1)

        if echo "$line" | grep -q "DONE"; then
            if echo "$line" | grep -q "fallback"; then
                printf "║  %-28s  %-10s  %-20s  ║\n" "${model}" "✅ DONE(8k)" "—"
            else
                printf "║  %-28s  %-10s  %-20s  ║\n" "${model}" "✅ DONE" "—"
            fi
