...
run_lm_eval() {
    ...
    export USAGE_LOG_PATH="/root/forkGO/semantic-routing/evals/stage8_large/mistral-small3.2_usage.jsonl"
    ...
    PYTHONPATH="/root/forkGO/semantic-routing/evals" \
      ...
      --output_path '/root/forkGO/semantic-routing/evals/stage8_large/mistral-small3.2' \
      --log_samples \
      --batch_size 1 \
      --apply_chat_template \
      --trust_remote_code \
      --system_instruction 'For multiple choice questions, end your response with "the answer is (X)" where X is the letter.' \
      2>&1 | tee '/root/forkGO/semantic-routing/evals/stage8_large/mistral-small3.2.log'
}
