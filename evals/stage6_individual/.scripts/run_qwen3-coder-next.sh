...
export USAGE_LOG_PATH="/root/forkGO/semantic-routing/evals/stage6_individual/qwen3-coder-next_usage.jsonl"
...
PYTHONPATH="/root/forkGO/semantic-routing/evals" \
  ...
  --output_path '/root/forkGO/semantic-routing/evals/stage6_individual/qwen3-coder-next' \
  --log_samples \
  --batch_size 1 \
  --apply_chat_template \
  --trust_remote_code \
  --system_instruction 'For multiple choice questions, end your response with "the answer is (X)" where X is the letter.' \
  2>&1 | tee '/root/forkGO/semantic-routing/evals/stage6_individual/qwen3-coder-next.log'

echo ""
echo "=== DONE: qwen3-coder-next ==="

# Generate per-model cost summary
if [[ -f "/root/forkGO/semantic-routing/evals/stage6_individual/qwen3-coder-next_usage.jsonl" ]]; then
    python3 "/root/forkGO/semantic-routing/evals/summarize_costs.py" \
        --usage "/root/forkGO/semantic-routing/evals/stage6_individual/qwen3-coder-next_usage.jsonl" \
        --results-dir "/root/forkGO/semantic-routing/evals/stage6_individual/qwen3-coder-next" \
        --output "/root/forkGO/semantic-routing/evals/stage6_individual/qwen3-coder-next_cost_summary.json" \
