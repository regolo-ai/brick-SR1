...
run_lm_eval() {
    ...
    export USAGE_LOG_PATH="/root/forkGO/semantic-routing/evals/stage8_large_v2/brick_usage.jsonl"
    ...
    PYTHONPATH="/root/forkGO/semantic-routing/evals" \
      ...
      --model_args "model=brick,base_url=http://213.171.186.210:8000/v1/chat/completions,tokenizer_backend=huggingface,tokenizer=Qwen/Qwen2.5-72B-Instruct,stream=false,max_tokens=${MAX_TOKENS},temperature=0,top_p=1" \
      --tasks brick_large \
      --include_path '/root/forkGO/semantic-routing/evals/custom_tasks' \
      --output_path '/root/forkGO/semantic-routing/evals/stage8_large_v2/brick' \
      --log_samples \
      --batch_size 1 \
      --apply_chat_template \
      --trust_remote_code \
      --system_instruction 'For multiple choice questions, end your response with "the answer is (X)" where X is the letter.' \
      2>&1 | tee '/root/forkGO/semantic-routing/evals/stage8_large_v2/brick.log'
}
