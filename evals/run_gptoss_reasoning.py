#!/usr/bin/env python3
"""Eval gpt-oss models with thinking=True, reasoning_effort=high on brick_hard (200 MMLU-Pro questions).

Usage:
    python run_gptoss_reasoning.py gpt-oss-120b
    python run_gptoss_reasoning.py gpt-oss-20b
"""
...
OUT_DIR         = Path(__file__).parent / "stage6_individual_hard_v5" / f"{safe_name}-reasoning"
SYSTEM_PROMPT   = (
...
log_path     = OUT_DIR / f"{safe_name}-reasoning.log"
...
tee(f"Model: {MODEL}  thinking=True  reasoning_effort={REASONING_EFFORT}  questions={total}")
tee("=" * 60)
