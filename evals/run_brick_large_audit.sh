#!/usr/bin/env bash
# run_brick_large_audit.sh — Run brick_large (2000 questions) via audit-test container (port 8001)
# Launches a dedicated tmux session: large-audit-brick
# Results → evals/stage8_large_audit/brick/
set -eo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
EVALS_DIR="${SCRIPT_DIR}"
VENV_PYTHON="/root/forkGO/semantic-routing/.venv/bin/python3"
BRICK_URL="http://213.171.186.210:8001/v1/chat/completions"
REGOLO_API_KEY="${REGOLO_API_KEY:?ERROR: REGOLO_API_KEY not set}"
TOKENIZER="Qwen/Qwen2.5-72B-Instruct"
STAGE_DIR="${EVALS_DIR}/stage8_large_audit"
TMPDIR_SCRIPTS="${STAGE_DIR}/.scripts"
SESSION="large-audit-brick"
...
echo "[LAUNCH] Brick Audit → tmux attach -t ${SESSION}"
