#!/usr/bin/env bash
# run_individual_models_large_v2.sh — Run brick_large (2000 MMLU-Pro questions, 400/category)
# Tests the 2 new models individually: qwen3.5-122b, mistral-small-4-119b
# Launches ONE dedicated tmux session per model for easy parallel monitoring.
# Results → evals/stage8_large_v2/{model}/
set -eo pipefail
...
echo ""
echo "=== brick_large v2 evals launched (2000q, 400/cat, 2 new models, max_tokens=16384) ==="
echo "Results dir: ${STAGE_DIR}/"
echo ""
echo "Monitor individual sessions:"
for model_id in "${MODELS[@]}"; do
    safe_name="${model_id//\//_}"
    echo "  tmux attach -t large-v2-${safe_name}"
done
